#!/usr/bin/env bash
# install.sh — OpenClaw Brain one-liner installer
# Usage: curl -fsSL https://raw.githubusercontent.com/YOUR_USERNAME/openclaw/main/install.sh | bash
#
...
GPG_KEY_URL="https://YOUR_USERNAME.github.io/openclaw/apt/openclaw.gpg"

RED='\033[0;31m'; GRN='\033[0;32m'; YLW='\033[1;33m'; BLU='\033[0;34m'; NC='\033[0m'
info()  { echo -e "${BLU}[openclaw]${NC} $*"; }
ok()    { echo -e "${GRN}[openclaw]${NC} ✓ $*"; }
warn()  { echo -e "${YLW}[openclaw]${NC} ⚠ $*"; }
die()   { echo -e "${RED}[openclaw]${NC} ✗ $*"; exit 1; }

banner() {
    echo ""
    echo "  ╔══════════════════════════════════════╗"
    echo "  ║   OpenClaw Brain  Installer  v2.0   ║"
...
install_deb() {
    ...
    if curl -fsSL "$GPG_KEY_URL" &>/dev/null; then
        curl -fsSL "$GPG_KEY_URL" | sudo gpg --dearmor -o /usr/share/keyrings/openclaw.gpg
        echo "deb [signed-by=/usr/share/keyrings/openclaw.gpg] $APT_REPO stable main" \
            | sudo tee /etc/apt/sources.list.d/openclaw.list > /dev/null
        sudo apt-get update -qq
        if sudo apt-get install -y openclaw 2>/dev/null; then
...
install_rpm() {
    ...
    if command -v dnf &>/dev/null; then
        if sudo dnf copr enable -y "${GITHUB_REPO/\//\/}" 2>/dev/null; then
            sudo dnf install -y openclaw 2>/dev/null && { ok "Installed via dnf"; return; }
        fi
    fi
...
install_arch() {
    ...
    if command -v yay &>/dev/null; then
        yay -S --noconfirm openclaw 2>/dev/null && { ok "Installed via yay"; return; }
    elif command -v paru &>/dev/null; then
        paru -S --noconfirm openclaw 2>/dev/null && { ok "Installed via paru"; return; }
    fi
...
install_macos() {
    ...
    if brew tap "YOUR_USERNAME/openclaw" 2>/dev/null && \
       brew install --cask openclaw 2>/dev/null; then
    ...
    local plist="$HOME/Library/LaunchAgents/io.openclaw.plist"
    if [[ ! -f "$plist" ]]; then
        curl -fsSL "${GITHUB_RAW}/install/io.openclaw.plist" -o "$plist" 2>/dev/null || true
...
_post_install_linux() {
    ...
    if command -v systemctl &>/dev/null; then
        ...
        mkdir -p "$svc_dir"
        cat > "$svc_dir/openclaw.service" << 'SVC'
[Unit]
Description=OpenClaw Brain
...
SVC
        ...
        ok "Systemd service enabled (openclaw.service)"
    fi
}
...
post_install() {
    echo ""
    if command -v openclaw &>/dev/null || python3 -c "import interface.cli" 2>/dev/null; then
        ok "OpenClaw Brain installed successfully!"
    else
        warn "openclaw not found in PATH — you may need to restart your shell"
    ...
    echo "  ║   CLI:      openclaw \"your question\"   ║"
    echo "  ║                                         ║"
    echo "  ║   GitHub:                               ║"
    echo "  ║   github.com/${GITHUB_REPO}    ║"
    echo "  ╚═════════════════════════════════════════╝"
